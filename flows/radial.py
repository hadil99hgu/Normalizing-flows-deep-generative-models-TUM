...
class Radial(Flow):
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        ##########################################################
        alpha = F.softplus(self.pre_alpha).detach()
        beta = -alpha + F.softplus(self.pre_beta).detach()

        r = torch.norm(x - self.x0, dim=1)
        h = 1.0 / (alpha + r)
        y = x + beta * h[:, None] * (x - self.x0)
        log_det_jac = (D-1) * torch.log(1 + beta * h) + torch.log(1 + beta * h + beta * r * (-1/(alpha+r)**2))
